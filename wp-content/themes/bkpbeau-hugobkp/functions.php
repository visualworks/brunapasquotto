...
function beau_hugo_scripts(){
    ...
    // var_dump($beau_option);
    if (isset($beau_option['enable-fixed'])) {
        if ($beau_option['enable-fixed'] =='2') {
...
function beau_register_sidebar() {
    ...
    $col = $sidebarWidgets = "";
    if ($beau_option) {
        $col    = intval($beau_option['footer-columns']);
    }
    if($col==0){
        $col  = 4;
    }
    $custom         = $beau_option['custom_sidebar'];
    $sidebarWidgets = intval($beau_option['sidebar_numbers']);
